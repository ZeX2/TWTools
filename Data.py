...
class TWData(object):
	...
	def get_villages_dict(villages_data):
		...
		#Opens the data file (.txt.gz) and splits it into an array based on each line
		with gzip.open(villages_data) as f:
			data = f.read()
			villages_list = data.splitlines()

			#Each line in the array has data about the villages in the format: "id, name, x, y, player, points"
			for village in villages_list:
				...
				village_array = [village_id, village_name, village_x, village_y, village_player_id, village_points]
	...
	def get_players_dict(players_data):
		...
		with gzip.open(players_data) as f:
			...
			for player in players_list:
				...
				player_array = [player_id, player_name, player_ally_id, player_villages, player_points, player_rank]
